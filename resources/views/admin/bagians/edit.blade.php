...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('bagian.update',$bagian->id) }}" enctype="multipart/form-data">
                @method('patch')
                @csrf
                <div class="form-group mb-3">
                    <label for="nama">Nama</label>
                    <input class="form-control {{ $errors->has('nama') ? 'is-invalid' : '' }}" type="text"
                        name="nama" id="nama" value="{{ old('nama',$bagian->nama) }}">
                    @if ($errors->has('nama'))
                ...
                <div class="form-group mb-3">
                    ...
                    <input class="form-control {{ $errors->has('grade') ? 'is-invalid' : '' }}" type="number"
                        name="grade" id="grade" value="{{ old('grade',$bagian->grade) }}">
                    @if ($errors->has('grade'))
                        <div class="invalid-feedback">
                            {{ $errors->first('grade') }}
                        </div>
