...
<div class="card">
    <div class="card-header">
        <h5>Tambah Bahan Produksi</h5>
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("produksi.ambilBahanStore", $produksi->id) }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group mb-3">
                ...
                <textarea class="form-control {{ $errors->has('keterangan') ? 'is-invalid' : '' }}" name="keterangan" id="keterangan" rows="3">{{ old('keterangan', '') }}</textarea>
...
<link rel="stylesheet" href="{{ asset('js/autocomplete.css') }}">
    <script>
        new Autocomplete('#autocompleteProduk', {
            search: input => {
                const url = "{{ url('admin/produkStok/api?q=') }}" + `${escape(input)}`;
